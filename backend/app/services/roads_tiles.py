...
ROADS_MBTILES = Path(
    _env_value(
        "CARTOSKY_ROADS_MBTILES",
        default=str(DATA_ROOT / "roads" / "v1" / "cartosky_roads.mbtiles"),
    )
)
ROADS_TILESET_ID = _env_value(
    "CARTOSKY_ROADS_TILESET_ID",
    default="cartosky-roads-v1",
)
ROADS_TILESET_NAME = _env_value(
    "CARTOSKY_ROADS_TILESET_NAME",
    default="CartoSky Roads v1",
)
ROADS_TILESET_PATH_VERSION = (
    _env_value(
        "CARTOSKY_ROADS_TILESET_VERSION",
        default="v1",
    ).strip().strip("/")
    or "v1"
)
...
ROAD_CACHE_HIT = "public, max-age=31536000, immutable"
ROAD_CACHE_MISS = "public, max-age=15"
...
def read_mbtiles_metadata(path: Path) -> dict[str, str]:
    try:
        ...
        logger.exception("Failed reading roads MBTiles metadata: %s", path)
        return {}
...
def lookup_mbtiles_tile(path: Path, *, z: int, x: int, y: int) -> bytes | None:
    ...
    try:
        ...
        logger.exception("Failed reading roads MBTiles tile z/x/y=%s/%s/%s from %s", z, x, y, path)
        return None
...
def build_roads_tilejson() -> dict[str, object]:
    metadata = read_mbtiles_metadata(ROADS_MBTILES)
    minzoom, maxzoom = mbtiles_min_max_zoom(metadata)
    bounds_raw = metadata.get("bounds", "-180,-85.0511,180,85.0511")
    center_raw = metadata.get("center", "-101.5,45.0,4")
    ...
    try:
        ...
        center_vals = [-101.5, 45.0, 4.0]

    tilejson: dict[str, object] = {
        "tilejson": "2.2.0",
        "name": metadata.get("name", ROADS_TILESET_NAME),
        "id": metadata.get("id", ROADS_TILESET_ID),
        "scheme": "xyz",
        "format": "pbf",
        "minzoom": minzoom,
        "maxzoom": maxzoom,
        "bounds": bounds,
        "center": center_vals,
        "tiles": [f"{TILES_PUBLIC_BASE_URL}/tiles/v3/roads/{ROADS_TILESET_PATH_VERSION}/{{z}}/{{x}}/{{y}}.mvt"],
    }
    ...
    if "vector_layers" in metadata:
        try:
            ...
            logger.warning("Invalid vector_layers metadata in %s", ROADS_MBTILES)
