...
def test_derive_aigfs_tmp850_anomaly_uses_raw_tmp850_component_and_era5_baseline(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setattr(
    ...
    ctx = FetchContext()
    var_spec = AIGFS_MODEL.get_var("tmp850_anom")
    var_capability = AIGFS_MODEL.get_var_capability("tmp850_anom")
    assert var_spec is not None
    assert var_capability is not None
    assert var_spec.selectors.hints["baseline_region"] == "na"
...
def test_derive_gefs_tmp850_anomaly_uses_mean_tmp850_component_and_era5_baseline(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setattr(
    ...
    var_capability = GEFS_MODEL.get_var_capability("tmp850_anom")
    assert var_spec is not None
    assert var_capability is not None
    assert var_spec.selectors.hints["base_component"] == "tmp850__mean"
    assert var_spec.selectors.hints["baseline_region"] == "na"
...
def test_derive_eps_tmp850_anomaly_uses_mean_tmp850_component_and_era5_baseline(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setattr(
    ...
    var_capability = EPS_MODEL.get_var_capability("tmp850_anom")
    assert var_spec is not None
    assert var_capability is not None
    assert var_spec.selectors.hints["base_component"] == "tmp850__mean"
    assert var_spec.selectors.hints["baseline_region"] == "na"
