...
class _FakeRunSchedulerPlugin:
    ...
    def get_var_capability(self, var_key: str):
        return types.SimpleNamespace(buildable=True)

    def get_var(self, var_key: str):
        return types.SimpleNamespace(id=var_key)

    def resolve_probe_var_key(self, probe_var: str | None):
        return probe_var or "tmp2m"


def test_run_scheduler_skips_success_cleanup_for_incomplete_run(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
