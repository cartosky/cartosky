"""Phase 3 — G1 antimeridian gates for the ECMWF global domain (native 4326).

The ECMWF mirror of ``test_aifs_global_antimeridian.py`` — fourth and last
model in the rollout. IFS open data is delivered as a 0.25° 0–360° EPSG:4326
global field, so it adopts the contract — and this suite — verbatim; the clean
module-level pieces of the GFS suite (the analytic field, the synthetic source
raster, the GeoJSON walker, the bump fixture geometry) are imported rather than
re-derived, and only the model-parameterised parts are restated here.

Note this is the *global* domain only: ECMWF's canonical grid stays 9 km
EPSG:3857 and no part of this file touches it (Change A framing).

Confirming the *live* ECMWF GRIBs are on that grid is prod-gate item G1, not
something this file can assert; what it does assert is that the index-roll
branch accepts that geometry and that a build over it is a pure longitude
roll — pinned by patching ``reproject`` to explode, because value equality is
vacuous on an aligned grid.

Cycle asymmetry (00/12z vs 06/18z) is a *schedule* property and is pinned in
``test_ecmwf_global_domain.py``; nothing here depends on the horizon.
...
def global_live(monkeypatch: pytest.MonkeyPatch) -> None:
    """The Phase 3 prod flip: ECMWF allowlisted so `domain=global` validates."""
...
def _publish_global_frame(data_root: Path, values: np.ndarray, transform: object) -> Path:
    """Write the warped array into `published/ecmwf/domains/global/…`.
    ...
    (var_dir / "fh000.json").write_text(
        json.dumps({"fh": 0, "units": "F", "valid_time": "2026-08-03T00:00:00Z"})
    )
    build_grid_manifests_for_run_root(
        ...
        json.dumps(
            {
                ...
                "variables": {
                    VAR: {
                        ...
                        "expected_frames": 1,
                        "available_frames": 1,
                        "frames": [{"fh": 0, "valid_time": "2026-08-03T00:00:00Z"}],
                    }
                },
            }
        )
...
def test_the_roll_path_claims_this_source() -> None:
    """The index-roll branch must accept the ECMWF global source geometry.

    IFS open data is on the same 0.25° 0–360° global grid as GFS, so the
    ...
    assert rolled is not None, "index-roll branch rejected the native ECMWF global source"
...
def _warped_bump_field() -> tuple[np.ndarray, object]:
    ...
    one control over North America), warped onto the ECMWF global grid."""
    step = SRC_STEP_DEG
