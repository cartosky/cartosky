"""Phase 3 — G1 antimeridian gates for the AIGFS global domain (native 4326).

The AIGFS mirror of ``test_gfs_global_antimeridian.py``. AIGFS publishes the
same 0.25° 0–360° EPSG:4326 source as GFS (plugin docstring + the NOAA product
inventory), so it adopts the contract — and this suite — verbatim; the clean
module-level pieces of the GFS suite (the analytic field, the synthetic source
raster, the GeoJSON walker, the bump fixture geometry) are imported rather
than re-derived, and only the model-parameterised parts are restated here.

Confirming the *live* AIGFS GRIBs are on that grid is prod-gate item G1, not
...
def global_live(monkeypatch: pytest.MonkeyPatch) -> None:
    """The Phase 3 prod flip: AIGFS allowlisted so `domain=global` validates."""
...
def _publish_global_frame(data_root: Path, values: np.ndarray, transform: object) -> Path:
    """Write the warped array into `published/aigfs/domains/global/…`.
...
def test_the_roll_path_claims_this_source() -> None:
    """The index-roll branch must accept the AIGFS global source geometry.

    AIGFS publishes on the same 0.25° 0–360° global grid as GFS, so the same
    branch applies. Values alone cannot prove this: on a grid-aligned source
    ...
    assert rolled is not None, "index-roll branch rejected the native AIGFS global source"
...
def _warped_bump_field() -> tuple[np.ndarray, object]:
    ...
    one control over North America), warped onto the AIGFS global grid."""
    step = SRC_STEP_DEG
