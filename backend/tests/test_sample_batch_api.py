...
async def test_sample_batch_rejects_products_without_sampling_support(
    ...
    def fail_resolve_binary_frame(*args: object, **kwargs: object) -> Path:
    ...
    monkeypatch.setattr(main_module, "_resolve_binary_grid_frame", fail_resolve_binary_frame)
...
async def test_sample_missing_frame_returns_404(client: httpx.AsyncClient) -> None:
    # fh=2 is never published. Body text is pinned verbatim — clients match on
    # it (the "val.cog.tif" wording predates the binary substrate).
    missing = await client.get("/api/v4/sample", params={**SAMPLE_QUERY, "fh": 2})
    assert missing.status_code == 404
    assert missing.content == b'{"error": "val.cog.tif not found"}'

    batch_missing = await client.post(
        "/api/v4/sample/batch",
        json={
            "model": "hrrr",
            "run": "latest",
    ...
    assert batch_missing.status_code == 404
    assert batch_missing.content == missing.content
...
async def test_sample_batch_reuses_cached_payload(
    ...
    call_count = 0
    original = main_module.sample_binary_batch_values

    def wrapped(*args: object, **kwargs: object) -> dict[str, float | None]:
        nonlocal call_count
        call_count += 1
        return original(*args, **kwargs)

    monkeypatch.setattr(main_module, "sample_binary_batch_values", wrapped)
