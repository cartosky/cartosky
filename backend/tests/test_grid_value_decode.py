"""Layer 1 of the value-COG → grid-binary sampling migration (Phase A).
...
Structured to be parameterized by model (Phase G reuses this file for the next
model rather than duplicating it); GFS is the only model in scope for this phase.
"""
...
GFS_VARS = _vars_for_model(MODEL)
...
def test_gfs_scope_includes_loop_registered_precip_anomaly_vars() -> None:
    ...
    for var in GFS_PRECIP_ANOM_VARS:
        assert (MODEL, var) in _PACKING_BY_MODEL_VAR, (
            f"{MODEL}/{var} missing from _PACKING_BY_MODEL_VAR — scope regression"
        )


@pytest.mark.parametrize("var", GFS_VARS)
def test_in_range_values_round_trip_within_half_scale(var: str) -> None:
    ...
    assert np.all(np.abs(decoded - values) <= tol), (
        f"{var}: round-trip exceeded {tol}\n"
        f"values={values}\ndecoded={decoded}\ndiff={np.abs(decoded - values)}"
    )


@pytest.mark.parametrize("var", GFS_VARS)
def test_out_of_range_values_decode_to_clipped_result(var: str) -> None:
...
@pytest.mark.parametrize("var", GFS_VARS)
def test_nodata_sentinel_decodes_to_nan(var: str) -> None:
...
@pytest.mark.parametrize("var", GFS_VARS)
def test_decode_preserves_shape(var: str) -> None:
