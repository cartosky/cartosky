...
async def test_admin_build_events_returns_newest_first_with_filters(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _create_session(session_id="admin-session", member_id=42, name="Admin")

    frozen_now = 1_717_800_000
    ...
    with admin_telemetry._connect() as conn:
        conn.executemany(
            ...
            """,
            [
                (frozen_now - (20 * 24 * 60 * 60), "hrrr", "hrrr_14z_old", "14", 30.0 * 60.0),
                (frozen_now - (10 * 24 * 60 * 60), "gfs", "gfs_00z", "00", 45.0 * 60.0),
                (frozen_now - (5 * 24 * 60 * 60), "hrrr", "hrrr_14z", "14", 35.2 * 60.0),
                (frozen_now - 100, "hrrr", "hrrr_15z", "15", 40.0 * 60.0),
            ],
        )

    response = await client.get(
        "/api/v4/admin/build-events?model_id=hrrr&cycle_hour=14&days=30&limit=10",
        cookies={twf_oauth.SESSION_COOKIE_NAME: "admin-session"},
    )
    ...
    assert response.json() == [
        {
            "created_at": frozen_now - (5 * 24 * 60 * 60),
            "model_id": "hrrr",
            "cycle_hour": "14",
            "run_id": "hrrr_14z",
        ...
        {
            "created_at": frozen_now - (20 * 24 * 60 * 60),
            "model_id": "hrrr",
            "cycle_hour": "14",
            "run_id": "hrrr_14z_old",
...
async def test_admin_build_events_applies_days_and_limit_and_requires_admin(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _create_session(session_id="admin-session", member_id=42, name="Admin")
    ...
    with admin_telemetry._connect() as conn:
        conn.executemany(
            ...
            """,
            [
                (frozen_now - (40 * 24 * 60 * 60), "hrrr", "too_old", "12", 10.0 * 60.0),
                (frozen_now - 100, "hrrr", "latest_one", "12", 20.0 * 60.0),
                (frozen_now - 200, "gefs", "latest_two", "00", 25.0 * 60.0),
            ],
        )

    response = await client.get(
        "/api/v4/admin/build-events?days=30&limit=1",
        cookies={twf_oauth.SESSION_COOKIE_NAME: "admin-session"},
    )
...
async def test_admin_network_diagnostics_summary_groups_by_cache_model_and_device(client: httpx.AsyncClient) -> None:
    ...
    payloads = [
        ...
        },
        {
            ...
            "viewport_bucket": "lg",
            "page": "/viewer",
            "meta": {"webgl_backend": "webgl2", "payload_bytes": 8480000},
        },
        {
            ...
            "viewport_bucket": "sm",
            "page": "/viewer",
            "meta": {"webgl_backend": "webgl1", "payload_bytes": 512000},
        },
    ]
    ...
    assert upload_metric["by_payload_size_bucket"][0]["key"] == "8MB-16MB"
