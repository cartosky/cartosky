...
**Post-completion follow-ups landed 2026-07-08 — see §10.** The compare/diff mode audit (tracked separately) surfaced three real bugs in the systems this plan built: server-render viewport mismatch (compare + viewer screenshots clipped relative to the poster's actual view), split-mode screenshots missing city values (the exact failure class Phase 1 fixed for diff mode, never extended to split), and a mislabeled compare-screenshot overlay title when the two panels' runs differ. None of these reopen a phase or change the architecture; they are targeted fixes against already-shipped code, documented in §10 so the reference tables and Phase 1/4 history stay accurate.
...
- [x] **Capture render-wait hang fix (2026-07-07, post-ship):** symptom — GIF generation "incredibly slow" + slider preview stops updating, across models, cleared only by hard refresh. Root cause: `captureCanvasSnapshot` and `captureDraftDataUrl` in `map-canvas.tsx` awaited `map.once("render")` with **no timeout**; when `triggerRepaint()`'s rAF-scheduled render doesn't fire (throttled/backgrounded rAF, GPU hiccup) the capture promise never resolves, so the generate loop's `await captureFrame` hangs forever — its own 4s retry deadline never runs because control never returns from the await. Because `runningRef` stays true, `previewFrame` early-returns, so the frozen slider preview is the *same* bug, not a second one. Fix: added a 2000ms timeout to both capture paths (resolve `null` → caller retries/skips/falls back), mirroring the compare capture's existing 2s guard. Happy path unchanged. Optional not-yet-done follow-up: an overall generation wall-clock budget so a fully-frozen pipeline fails fast rather than grinding through per-frame skip timeouts.
...
- [x] **Post-gate polish round 2 (2026-07-07):**
  ...
  - Chrome trim (revised after prod compare screenshot showed dominant cards): `composeShareFrame` now scales chrome by the **limiting dimension** — `min(width/1280, height/720) × CHROME_COMPACT_FACTOR (0.72)`. The old width-only derivation drew ~1.56× cards on wide-short compare split composites (≈2000×760) that the frame height couldn't accommodate; they now land at ~0.76 (≈76% smaller by area). Viewer stills: 0.72. GIF frames pass an explicit `GIF_CHROME_SCALE = 0.5` (prod gate feedback: 0.55 still read large; 0.5 = 8px overlay text, the hard legibility floor through 256-color quantization), used as-is by the composer.
...
| Server headless render, poster-viewport clamp | `backend/app/services/screenshot_service.py` |
| Compare-mode screenshot state, per-side city-label gate | `frontend/src/pages/compare.tsx` |
| Split-panel city-label callback plumbing | `frontend/src/components/compare/ComparePanel.tsx` |

## 10. Post-completion follow-ups (2026-07-08)

Found and fixed during the separate compare/diff mode audit (see `docs/COMPARE_DIFFERENCE_MODE_DESIGN.md` / project memory `project_compare_diff_audit.md`), not a reopening of this plan — recorded here because each fix touches a system this plan built and Phase 1/4's history would otherwise go stale.

- **Server-render viewport pass-through (fixes clipped compare/viewer screenshots).** Root cause: `screenshot_service.py` always rendered at a fixed 1280×720 Playwright viewport, but a permalink's `lat/lon/z` only reproduces the poster's view **at the same canvas size** — visible extent scales with canvas pixels at fixed zoom. A poster on a ~1900px window got a screenshot showing significantly less area on every edge than their own screen (reported independently against both `/compare` split and diff, and the same mechanism affects the main viewer). Fix: the client (`useScreenshotCapture.ts`) now POSTs `viewport: { width: window.innerWidth, height: window.innerHeight }` to `/api/v4/share/screenshot`; the route passes it through to `screenshot_service.render()`, which clamps it via a new `resolve_render_viewport()` (960–2560 × 540–1440; portrait or sub-minimum windows — i.e. phones — fall back to the 1280×720 default **on purpose**, since normalizing mobile captures to a forum-friendly landscape frame is the reason this server path exists per §3.1/§3.5). Desktop landscape posters now get true WYSIWYG server renders; mobile normalization is unchanged. Considered and rejected: fitting the fixed 1280×720 viewport to the map's bounds instead of matching window size — that lowers zoom to fit, which risks dropping below the city-label minimum zoom (the exact failure mode below), re-breaking what this section's next fix repairs.
- **Split-mode city-value labels missing from screenshots.** Phase 1 (§2.2, §3.4) fixed this for **diff** mode by adding an `onCityLabelsReady` gate step so `data-compare-ready` waits for city labels in addition to the frame-ready signals — split mode never got the equivalent, so split-mode TWF screenshots intermittently shipped without city values (a race: the headless capture could fire before per-city sampling finished). Fix: `ComparePanel` now accepts and forwards `onCityLabelsReady` from `MapCanvas`; `compare.tsx` wires `handleLeftCityLabelsReady`/`handleRightCityLabelsReady` refs into the split-mode readiness gate alongside the existing per-panel frame-ready refs, so `data-compare-ready` now requires left-frame AND right-frame AND left-city-labels AND right-city-labels in split mode — matching the diff-mode gate shape exactly.
- **Compare screenshot overlay title showed only the left panel's run.** `buildShareScreenshotState` built `model: "{LEFT} vs {RIGHT}"` but `run: leftLoader.resolvedRun` only, so a screenshot comparing e.g. 12Z GFS against 00Z GFS stamped a single run id that was only ever true for the left panel — actively misleading when runs differ (the two panels show different valid times under one claimed run). Fix: `ScreenshotExportState` gained an optional `overlayTitle` field that `defaultOverlayLines` (`screenshot_export.ts`) uses in place of the default `{run} {model}` prefix when present; `compare.tsx` builds it per-side via the existing `formatRunModelSide` helper — split mode emits `Compare: {L run} {L model} vs {R run} {R model}`, diff mode emits `Difference: {L run} {L model} − {R run} {R model}` — so both panels' actual runs are always visible, and the "Compare:"/"Difference:" prefix also now distinguishes these screenshots from an ordinary single-model viewer share at a glance.

All three verified against the live dev backend (server render, split/diff readiness gates, and overlay text respectively) and the viewport fix additionally prod-confirmed by the operator.
