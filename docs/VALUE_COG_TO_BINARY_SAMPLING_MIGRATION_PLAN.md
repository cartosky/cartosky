...
**Status:** Migration in progress. Six models (GFS, HRRR, NBM, GEFS, EPS, AIFS) are live on `CARTOSKY_BINARY_SAMPLING_MODELS` with value-COG writes off; post-flip verification complete for GEFS/EPS (2026-07-05) and AIFS (2026-07-05, see Phase G audit below). ECMWF audit complete, canary complete with one classifier-gap finding (accepted as-is, not a data bug), Phase C evidence and storage measurement in progress as of this revision. Remaining post-migration items: per-variable **binary** footprint measurement on a post-cutover run per model (feeds `docs/ENSEMBLE_MEMBER_PIPELINE_PLAN.md` Phase 0), and the passive storage reclaim as COG-era runs age out of retention (~24–36 h per model from its flip; AIFS's reclaim is ~79 GB, by far the largest single-model reclaim in this migration).
