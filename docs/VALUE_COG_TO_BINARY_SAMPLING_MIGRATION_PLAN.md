...
**Status:** Migration in progress. Seven models (GFS, HRRR, NBM, GEFS, EPS, AIFS, ECMWF) are live on `CARTOSKY_BINARY_SAMPLING_MODELS` with value-COG writes off. AIGFS flip in progress (deterministic model, compressed process, per-operator decision — see note below). NDFD and WPC are structurally different from every model above (standalone poller-driven publishers, not `scheduler.py`/`pipeline.py`-routed) and are mid-migration under a dedicated phased plan — see the NDFD/WPC section below; **do not add `ndfd`/`wpc` to `CARTOSKY_BINARY_SAMPLING_MODELS` until that section's Phase 4 is reached**, since their publishers do not yet skip the value-COG write and their gate is shadow-only. SPC Outlooks, CPC Outlooks, and NWS Hazards are confirmed vector/polygon products with no packed grid artifact — permanently out of this migration's scope, not pending. Remaining post-migration items: per-variable **binary** footprint measurement on a post-cutover run per model (feeds `docs/ENSEMBLE_MEMBER_PIPELINE_PLAN.md` Phase 0), and the passive storage reclaim as COG-era runs age out of retention per model (ECMWF's reclaim, ~103 GB, is currently the largest single-model reclaim in this migration).
...
| Real canary run (Layer 3 equivalent) | **Not started** | **Not started** |
| Phase 4 — enforce + COG-skip + allowlist flip | **Blocked** on the canary run above | **Blocked** on the canary run above |

**Remaining sequence:** one real canary pass per model (single run, no 4-cycle requirement — 100% Group 1, no cycle-hour dependency, same compressed treatment AIFS received), now unblocked by the `RUN_ID_RE` fix; zero divergence expected given the shared, already-proven encode path, but this is the first time it will actually be checked rather than assumed. Then Phase 4: change both publishers' shadow gate calls to enforced (reject on failure, matching `pipeline.py`'s `binary_only` branch), add the COG-write skip conditional, remove reliance on the skipped Phase 1 guard (there is none to remove — note this explicitly if Phase 1 is revisited later), flip the allowlist, restart both poller processes (`csky-ndfd-scheduler`, `csky-wpc-scheduler` — not `sudo systemctl restart csky-*-scheduler.service` blindly assumed; confirmed these are the real unit names), and run the standard post-flip verification (COG-write skip on next cycle, `/api/v4/sample` spot-check, meteogram spot-check for at least one NDFD in-app-derived variable and WPC's cumulative `precip_total`).
