...
**Status:** Migration in progress. Nine models (GFS, HRRR, NBM, GEFS, EPS, AIFS, ECMWF, NDFD, WPC) are live on `CARTOSKY_BINARY_SAMPLING_MODELS` with value-COG writes off. AIGFS flip status unconfirmed in this record — verify before assuming it landed. NDFD/WPC required a dedicated phased plan (standalone poller-driven publishers, not `scheduler.py`/`pipeline.py`-routed) rather than the standard checklist — see the NDFD/WPC section below for the full record, including a critical shadow-gate bug found and fixed before it reached production and a real (non-blocking) latency finding on NDFD. SPC Outlooks, CPC Outlooks, and NWS Hazards are confirmed vector/polygon products with no packed grid artifact — permanently out of this migration's scope, not pending. Remaining post-migration items: per-variable **binary** footprint measurement on a post-cutover run per model (feeds `docs/ENSEMBLE_MEMBER_PIPELINE_PLAN.md` Phase 0), and the passive storage reclaim as COG-era runs age out of retention per model (ECMWF's reclaim, ~103 GB, is currently the largest single-model reclaim in this migration; NDFD/WPC's combined ~4.8 GB is the smallest).
...
**Flip, completed 2026-07-09.** `CARTOSKY_BINARY_SAMPLING_MODELS` now includes `ndfd, wpc` (full list: `gfs, hrrr, nbm, gefs, eps, aifs, ecmwf, ndfd, wpc`); `csky-api`, `csky-ndfd-scheduler`, and `csky-wpc-scheduler` restarted. **This closes the last two models in this migration's originally-scoped model set** — the only remaining out-of-scope items are the confirmed-permanent vector-product exclusions (SPC/CPC/NWS Hazards) and AIGFS's flip status, which is unconfirmed in this record and should be checked rather than assumed complete. Post-flip verification (COG-write skip on next poll cycle for both, `/api/v4/sample` spot-checks for a raw and a derived NDFD variable plus WPC's cumulative `precip_total`, meteogram spot-check) was specified but its results are not yet recorded in this document — confirm and update this line once run, per this document's own standing convention against leaving verification steps unrecorded.
