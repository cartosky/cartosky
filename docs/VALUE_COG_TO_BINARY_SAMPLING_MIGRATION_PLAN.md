...
**Status:** Migration complete. All five in-scope models (GFS, HRRR, NBM, GEFS, EPS) are live on `CARTOSKY_BINARY_SAMPLING_MODELS` with value-COG writes off; post-flip verification complete for all (GEFS/EPS 2026-07-05, see Phase G closure). Remaining post-migration items: per-variable **binary** footprint measurement on a post-cutover run per model (feeds `docs/ENSEMBLE_MEMBER_PIPELINE_PLAN.md` Phase 0), and the passive storage reclaim as COG-era runs age out of retention (~24–36 h per model from its flip).
