...
export function AdminHero(props: {
  ...
  return (
    <section className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#0b1526]/92 px-5 py-5 text-white shadow-[0_20px_60px_rgba(0,0,0,0.24)] md:px-6">
      <div
        aria-hidden="true"
...
export function AdminStat(props: {
  ...
  const { label, value, hint, accentClassName = "text-white", icon, className, status, onClick } = props;
  const content = (
    <div className={cn("rounded-[1.15rem] border border-white/8 bg-white/[0.025] px-4 py-4 transition-colors", onClick ? "hover:bg-white/[0.045]" : "", className)}>
